...
namespace System.Web.Mvc
{
    ...
    public static class My97DatePicker
    {
        ...
        private static string GenerateHtml(string name, DateTime? date, string format)
        {
            if (date != null)
            {
                //return string.Format("<input type='text' id='{0}' onclick='WdatePicker({el:'{0}'})' class='Wdate' value='{1}'/>", name, date.Value.ToString(format));
                return "<input type=\"text\" id=\"" + name + "\"  name=\"" + name + "\"  onclick=\"WdatePicker({el:'" + name + "'})\" class=\"Wdate form-control\" value=\"" + date.Value.ToString(format) + "\" />";
            }
            else
            {
               // return string.Format("<input type='text' id='{0}' onclick='WdatePicker({el:'{0}'})' class='Wdate' value=''/>", name);
                return "<input type=\"text\" id=\"" + name + "\"  name=\"" + name + "\"  onclick=\"WdatePicker({el:'" + name + "'})\" class=\"Wdate form-control\" value=\"\" />";
